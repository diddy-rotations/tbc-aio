...
do
	...
	local function should_maintain_lacerate(ctx)
		if not is_spell_available(A.Lacerate) then
			return false
		end
		if ctx.settings.maintain_lacerate == false then
			return false
		end
		if ctx.settings.lacerate_boss_only and not Unit(TARGET_UNIT):IsBoss() then
			return false
		end
		return true
	end
	...
	local function should_hold_for_mangle()
		if not is_spell_available(A.MangleBear) then
			return false
		end
	...
	local function is_targettarget_healer()
		if not _G.UnitExists("targettarget") then
			return false
		end
	...
	local function should_tab_target(ctx, state)
		...
		if ctx.enemy_count < 2 then
			return false
		end
		if _G.UnitIsPlayer(TARGET_UNIT) then
			return false
		end
		if Unit(TARGET_UNIT):CombatTime() == 0 then
			return false
		end

		-- Switch away from CC'd target to find a valid one
		if is_target_breakable_cc() then
			return true
		end

		-- Only switch if current target is valid and in melee range
		if
			_G.UnitExists(TARGET_UNIT)
			and not _G.UnitIsDead(TARGET_UNIT)
			and _G.UnitIsVisible(TARGET_UNIT)
			and ctx.in_melee_range
		then
			-- Only switch if we have solid aggro (threat >= 2) on current target
			local threat = Unit(TARGET_UNIT):ThreatSituation() or 0
			local hasAggro = threat >= 2
				or (_G.UnitExists("targettarget") and _G.UnitIsUnit("targettarget", PLAYER_UNIT))
			...
			for unitID in pairs(plates) do
				if
					...
					and A.MangleBear:IsInRange(unitID) == true -- Only consider targets in melee range (more reliable than GetRange)
				...
				then
					...
					if not unitIsDying then
						...
						local unitTargetingPlayer = _G.UnitExists(unitID .. "target")
							and _G.UnitIsUnit(unitID .. "target", PLAYER_UNIT)
	...
	cleu_frame:SetScript("OnEvent", function()
		local _, event, _, srcGUID, _, _, _, _, destName, _, _, p12, p13, p14, p15, _, _, _, _, p20 =
			_G.CombatLogGetCurrentEventInfo()
		if srcGUID ~= player_guid then
			return
		end
	...
	local function would_starve_mangle(ctx, rage_cost)
		...
		if cd >= 0.5 then
			return false
		end -- Mangle far off, safe to spend
		if (ctx.rage - rage_cost) >= RAGE_COST_MANGLE then
			return false
		end -- enough rage for both, safe to spend
		...
		if ctx.in_melee_range and not bear_state.maul_queued then
			...
			if swing_remaining > 0 and swing_remaining < cd then
				return false
			end
		end
	...
	local Bear_Enrage = {
		...
		matches = function(context)
			if
				not (context.rage < (context.settings.enrage_rage_threshold or Constants.BEAR.ENRAGE_RAGE_THRESHOLD))
			then
				return false
			end
	...
	local Bear_LacerateUrgent = {
		...
		execute = function(icon, context, state)
			...
			return try_cast_fmt(
				...
				"[P5]",
				"Lacerate URGENT",
				"5 stacks, Duration: %.1fs%s",
				state.lacerate_duration,
				cc_str
			)
		end,
	}
	...
	local Bear_Growl = {
		...
		spell = A.Growl,
		matches = function(context)
			if context.settings.bear_no_taunt then
				return false
			end
			...
			if is_target_cc_locked(Constants.BEAR.GROWL_CC_THRESHOLD) then
				return false
			end
			-- Skip if target is already attacking us (we have aggro)
			if has_target_aggro() then
				return false
			end
			...
			if not targeting_healer and context.ttd < Constants.BEAR.GROWL_MIN_TTD then
				return false
			end
			return true
		end,
	...
	local Bear_ChallengingRoar = {
		...
		execute = function(icon, context)
			local croar_range = context.settings.croar_range or Constants.BEAR.DEFAULT_CROAR_RANGE
			local elites, bosses = count_nearby_enemies(croar_range, true)
			local reason = bosses >= 1 and format("EMERGENCY - %d boss(es) loose, %d elite(s)", bosses, elites)
				or format("EMERGENCY - %d loose elite(s)", elites)
	...
	local Bear_DemoRoar = {
		...
		spell_target = PLAYER_UNIT,
		matches = function(context)
			if would_starve_maul(context, RAGE_COST_DEMO_ROAR) then
				return false
			end
			if would_starve_mangle(context, RAGE_COST_DEMO_ROAR) then
				return false
			end
			...
			if bosses < min_bosses and elites < min_elites and trash < min_trash then
				return false
			end
			...
			if demo_duration > Constants.BEAR.DEMO_ROAR_REFRESH then
				return false
			end
			...
			if shout_duration > Constants.BEAR.DEMO_ROAR_REFRESH then
				return false
			end
			return true
		end,
		execute = function(icon, context)
			...
			local elites, bosses, trash = count_nearby_enemies(demo_range, false)
			local cc_str = context.has_clearcasting and " [CC]" or ""
			local reason = bosses >= 1 and format("%d boss(es) + %d elite(s)", bosses, elites)
				or format("%d elite(s), %d trash", elites, trash)
			return try_cast_fmt(
				A.DemoralizingRoar,
				icon,
				PLAYER_UNIT,
	...
	local Bear_SwipeAoE = {
		...
		matches = function(context, state)
			...
			if context.enemy_count < aoe_threshold then
				return false
			end
			-- CC safety
			if context.settings.swipe_cc_check ~= false then
				if has_breakable_cc_nearby() then
					return false
				end
			end
			if not context.has_clearcasting then
				...
				if context.rage < swipe_threshold then
					return false
				end
				if would_starve_maul(context, RAGE_COST_SWIPE) then
					return false
				end
	...
	local Bear_Mangle = create_combat_strategy({
		...
		prefix = "[P9]",
		log_fmt = "Rage: %d%s",
		log_args = function(ctx)
			return ctx.rage, ctx.has_clearcasting and " [CC]" or ""
		end,
		extra_match = function(ctx)
			-- Skip if target has physical immunity
			if ctx.target_phys_immune then
				return false
			end
			...
			if ctx.rage < mangle_threshold then
				return false
			end
			return true
		end,
	...
	local Bear_Swipe = {
		...
		matches = function(context, state)
			...
			if should_hold_for_mangle() then
				return false
			end
			...
			if not context.has_clearcasting then
				...
				if would_starve_mangle(context, RAGE_COST_SWIPE) then
					return false
				end
			end
	...
	local Bear_LacerateBuild = {
		...
		matches = function(context, state)
			...
			if not context.has_clearcasting then
				if context.rage < RAGE_COST_LACERATE then
					return false
				end
				if would_starve_maul(context, RAGE_COST_LACERATE) then
					return false
				end
				if would_starve_mangle(context, RAGE_COST_LACERATE) then
					return false
				end
			end
			...
			return duration > Constants.BEAR.LACERATE_URGENT_REFRESH
				and duration <= Constants.BEAR.LACERATE_SWIPE_THRESHOLD
		end,
		execute = function(icon, context, state)
			local cc_str = context.has_clearcasting and " [CC]" or ""
			return try_cast_fmt(
				...
				"[P11]",
				"Lacerate",
				"Stacks: %d/5, Duration: %.1fs%s",
				state.lacerate_stacks,
				state.lacerate_duration,
				cc_str
			)
		end,
	...
	local Bear_Maul = {
		...
		matches = function(context, state)
			...
			if bear_state.maul_confirmed then
				return false
			end
